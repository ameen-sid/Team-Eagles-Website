...
const Player = (context: any) => {
	...
	const getPlayer = async () => {
		try {
			...
			const response = await axios.post(
				"/api/v1/global/get-full-player-details",
				{ playername }
			);
	...
	return (
		<>
			...
			<div
				id="container"
				className="bg-white w-auto flex lg:px-24 px-12 justify-center relative">
				<div
					id="container"
					className="p-10 border-2 sm:p-16 md:p-20 lg:p-24 xl:p-20 w-auto flex lg:flex-row flex-col gap-8 md:flex px-4 sm:px-8 md:px-24 lg:px-24 xl:px-24 relative">
					...
					<div className="flex flex-col items-center justify-center gap-10">
						...
						<div
							...
							<a
								rel="noopener"
								target="_blank"
								href={`https://${player.instagramLink}`}
								className="bg-black rounded-lg p-5 w-36 flex items-center gap-2 text-white">
								<FaInstagram size={24} />
								<span>Instagram</span>
							</a>
							<a
								rel="noopener"
								target="_blank"
								href={`https://${player.facebookLink}`}
								className="bg-black rounded-lg p-5 w-36 flex items-center gap-2 text-white">
								<FaFacebook size={24} />
								<span>Facebook</span>
							</a>
